...
class TinyModel(nn.Module):
    ...
    def __init__(self, input_size=2048, output_size=1000) -> None:
        ...
        self.layer1 = cs.Conv2d(3, 64, kernel_size=3, padding=1)
        self.flatten = cs.Flatten()
        self.layer2 = cs.Linear(64 ** 3, int(output_size))
...
def main(gpuCount, globalBatch, amplificationLimit=2.0, dataParallelBaseline=False, netBw=2.66E5, spatialSplit=False, simResultFilename=None, use_be=False):
    ...
    cs.computeInputDimensions((3, 64, 64))
    ...
    if not spatialSplit:
        cc = ClusterClient()
        jobName = "tiny2_%d_%d_%2.1f%s" % (gpuCount, globalBatch, amplificationLimit, "_DP" if dataParallelBaseline else "")
